...
shinyServer(function(input, output) {
  ...
  #     bakgr_sub <- bakgr[in_range(bakgr[[input$x_var]], input$x_range) &
  #                          in_range(bakgr[[input$y_var]], input$y_range), ]
  #     
  #     bakgr_sub
  #   }
  ...
  output$main_plot <- reactivePlot(function() {
    ...
    # Load dataset and create label vector
#    bakgr <- loadKLData(input$dataset)
    ...
    # Take a subset of the data, respecting the limited range
    #bakgr_sub <- limit_data_range()
    
    #if (is.null(bakgr_sub))
    #  return()
    
    # Get the x and y values from the non-range-limited data, for convenience
    #xdat <- bakgr[[input$x_var]]
    #ydat <- bakgr[[input$y_var]]
    ...
    # Base plot
    #    p <- ggplot(bakgr_sub, mapping = aes_mapping) +
    p <- ggplot(bakgr[bakgr$Kod == input$x_var,], aes(x=value, y=value)) + geom_point()
    #      points +
    #       theme_bw() +
  ...
  # ------------------------------------------------------------------
  # Create reactiveUI sliders for x and y range, because their limits
  # depend on the selected x and y variables.
  
  #   output$x_range_slider <- reactiveUI(function() {
  #     xmin <- floor(min(bakgr[[input$x_var]]))
  #     xmax <- ceiling(max(bakgr[[input$x_var]]))
  #     
  ...
  #   })
  #   
  #   output$y_range_slider <- reactiveUI(function() {
  #     ymin <- floor(min(bakgr[[input$y_var]]))
  #     ymax <- ceiling(max(bakgr[[input$y_var]]))
  #     
  ...
  #     do.call(model_type, args = list(formula = formula, data = quote(bakgr_sub), ...))
  #   }
  
  #   output$mod_linear_text <- reactivePrint(function() {
  ...
  #   output$mod_quadratic_text <- reactivePrint(function() {
  ...
  #   output$mod_loess_text <- reactivePrint(function() {
  #     formula <- paste(input$y_var, "~", input$x_var)
